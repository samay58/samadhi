...
The production track-fit and rotary-control slice passes formatter lint, 83 package tests, 11 app-model tests, 10 UI tests, and an exact-profile signed iPhone build. The paired phone was unavailable over the network at the installation step, so this exact build is not claimed as installed.
