...
The research, source-neutral planner, production track-fit connection, and aperture click wheel are complete. Adaptive runs preserve the first ready song, wait for real cadence, and may prepare a better fit after five seconds of stable mismatch. Identified callbacks prevent stale preparation from winning. The exact-profile harness proved an obvious 0.90 versus 1.10 change on one Bluetooth track. The current software candidate spans 0.85 through 1.15 at Samay's direction. Apple Music read-back and full-song listening at both new endpoints remain open. The next action is one exact-profile endpoint comparison followed by a clean walking run and one known natural transition.
