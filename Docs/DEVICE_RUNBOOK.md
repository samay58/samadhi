...
The build is suitable for product testing, not public distribution. Commit `cd07fd4` was built from a clean `main`, signed with the renewed exact profile, and installed on 2026-07-27 without uninstalling the prior app. The selected collection survived byte-for-byte. Foreground launch and physical process confirmation passed. Physical click-wheel feel, audible Auto response, and a natural prepared transition remain open.
...
Automatic token generation and 10 of 10 preview decodes pass with exact App ID signing. The current `Samadhi Development` profile expires on 2026-07-30 UTC. Repeat the renewal steps below before a later physical installation if it has expired.
