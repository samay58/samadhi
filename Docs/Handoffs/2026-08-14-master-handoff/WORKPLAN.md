...
- Phase 3: the transport and Finish control pass and the first directional Auto feedback prototype are complete in software. The three transport pills became one glass capsule bar, Finish became a quiet hold whose fill stays inside its own capsule, every song change carries a recorded cause, the reducer owns one cue transaction per meaningful Auto change, and three prototype families with paired arrival sounds play from a Debug audition screen. The physical comparison is open: press and hold feel, blinded direction recognition, size reading, sound quality against real Apple Music, ducking, locked-screen delivery, and engine recovery. No family, size scale, or sound path is chosen. Temporary screen copy is not part of the solution. Setup, summary, and icon craft remain later. Source: `original/pasted-text.txt:1291-1524`; `original/addendum-01-buttons.txt`; `original/addendum-02-workout-feedback.txt`; `original/addendum-03-transition-haptics-priority.txt`.
