...
## 2026-08-19. Walk repairs, the record and the boundary

- Changed the run record so per-second ticks are the only entries the buffer may drop: 2,048 entries, oldest tick first, song changes, rate writes, mode changes, cues, route events, and the new delivery, engine, plan, and reach entries kept for a whole run; schema 11, a deterministic eviction test, under 3 MB for an hour
- Made every cue report what became of it: played through the engine, local sound only, engine unavailable with the reason, pattern missing, or cancelled while held, each with transaction, moment, family, and sound path; the Core Haptics factory reports created, started, start failed, stopped with Apple's reason, reset, and unsupported; Debug builds write all of it into the run record and show the last three outcomes on the hidden Debug screen
- Added boundary look-ahead to the reducer: inside the last 45 seconds of a song the queued next song is judged against the settled Auto target and one better fit is prepared if it cannot follow, a kept choice is never traded for a marginal one, nothing fitting leaves the queue alone and says so, and the session tracks the collection index the queue continues after so a prepared song does not mislead the next judgment; the walk's own index order (39 then 1) confirmed that model
- Added the one reach line: fewer than a quarter of the ready songs reachable at the settled target for twenty seconds allows "Most of this playlist is faster than you're moving." or its slower twin once per run per direction, in the status slot under the dial, taken down by the shell; a scripted `-SAMADHI_OUT_OF_REACH` walk against a three-song demo set drives it in the Simulator
...
The transport and Finish pass, deterministic song-change causes, and the first directional Auto feedback prototype are merged on `main` and pass the software gate. The August 19 walk on the version 5 build is parsed under `Evidence/Device/2026-08-19-walk-easy-miles/`; the four software gaps it exposed (a lossy diagnostics buffer, no cue delivery record, no look-ahead at natural boundaries, nothing said about a playlist out of reach) are repaired on `main` and the repair build is installed on the phone; see STATUS for the commit and fingerprint. Endpoint read-back and listening at 0.85 and 1.15 carry over from the previous candidate, and tactile feel, sound quality, locked-screen delivery, and Apple Music coexistence have no result at all. The next physical step is the same walk again, ten minutes or more with Easy Miles, so the complete record can say which cues played; then the ordered checklist in [PHONE-CHECK-2026-08-19.md](PHONE-CHECK-2026-08-19.md) from step 2 onward, then the 20-minute outdoor run.
