...
## The run record evicts ticks, never the story

The diagnostic buffer held 512 entries and dropped the oldest regardless of kind. On the August 19 walk that cost the first 205 seconds, a whole song, and the only Auto changes Samay was describing. Schema 11 keeps 2,048 entries and, when the buffer is full, removes the oldest per-second tick (active second, raw cadence sample, cadence update, player progress) before anything else; only when no tick is left does a story entry go. Song changes, rate writes, mode changes, wheel turns, route and interruption events, cue transactions, cue deliveries, engine events, next-song plans, and reach notices therefore survive a run of any realistic length. At about 1.4 KB per pretty-printed entry the file stays under 3 MB for a 60-minute run. The price is that a long run's ticks cover only its last quarter hour or so; the changes are what the record is for.

## A cue is not played until something says it played

The reducer's `emitAutoFeedback` effect is a request, not a result. The walk sent five start cues and three arrivals and the file could not say whether one reached the hand. The shell now reports what became of every cue the service handled: played through the engine, played as local sound only, engine unavailable with the reason, pattern missing, or cancelled before it played, each with its transaction identifier, moment, family, and sound path. The Core Haptics factory also reports its engine lifecycle: created, started, start failed, stopped with Apple's reason, reset, unsupported. Debug builds write these into the run record and show the last three cue outcomes on the hidden Debug screen. This is still shell evidence, not product state: the reducer does not read it and no product rule depends on it. "Played" means the engine started the pattern; whether a pocket felt it remains a physical judgment.

## The boundary is judged before it arrives

The prepared-song path used to start only after the current song had sat at its rate limit for five seconds. That is the right rule while a song is playing, and it stays. It said nothing about the song about to play: at the walk's one natural boundary the music went from a fitting Numb into a 163.75 BPM Nightrider and pinned at 0.85 from its first second. Inside the last 45 seconds of a song the reducer now judges the queued next song against the settled Auto target. If that song cannot reach the target inside the rate window and something in the ready collection can, one better fit is prepared for the boundary; a plan that still fits is kept rather than traded for a marginally better one, because hunting between songs near a boundary is worse than either song. If nothing fits, the queue proceeds and the state says so. Skip and Previous stay the runner's and carry no plan.

To know which song is queued, the session keeps the collection index the player's queue continues after. A prepared song is slotted in after the song that was playing, so when it arrives the queue still resumes after that earlier song, and the anchor stays put; any other arrival moves the anchor with it. The walk's own record confirmed the shape: Numb at index 39 was followed by index 1, which only a prepared transition from index 0 produces.

## The collection's reach is said once

Auto held 0.85 for most of seven minutes and reported `atLimit` every second, and the screen said nothing, because per-change screen copy is forbidden and that rule stands. Reach is a different fact: it is about the collection, not a change. When fewer than a quarter of the ready songs can reach the settled Auto target inside the rate window, and that has held for twenty seconds, the reducer allows one line, once per run per direction: "Most of this playlist is faster than you're moving." or its slower twin. The shell shows it in the status slot under the dial, the same place the lock brief lives, and takes it down on its own. The words name the playlist and the body, never a limit, a rate, or a number. Manual takeover pauses the count, and a run that can follow the collection never hears the line. The thresholds are starting points and the reducer owns them.
