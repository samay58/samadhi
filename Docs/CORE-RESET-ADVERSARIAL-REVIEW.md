...
| F-015 transport and finish craft | Expanded user requirement | Haptics are insufficient. The buttons need a complete visual and interaction pass. The reported Finish border overflow is a concrete defect to reproduce and fix. |
...
Track-scoped Manual, safe cadence handling, the separate Auto target, walking support, approximate matching, and the quiet tempo-wheel close are complete in software. The exact-profile workout closed the immediate fingerprint and repaired-acquisition gate. The expanded 0.85 through 1.15 candidate is installed and awaits endpoint read-back and listening. If that check passes, the next Main Thing is transport and Finish craft. Directional Auto feedback remains later. Lifting-specific adaptation stays out. Warehouse, Run Set generation, setup, summary, and icon work remain later.
